...
async function call(path,method='GET',body,role='customer',extra={}){return worker.fetch(new Request('https://andallo.test/api/'+path,{method,headers:{'oai-authenticated-user-id':'test-owner',origin:'https://andallo.test','X-Andallo-Role':role,'X-Andallo-Email':role==='provider'?'provider@example.invalid':role==='admin'?'admin@example.invalid':'customer@example.invalid','Content-Type':'application/json',...extra},body:body===undefined?undefined:body instanceof Uint8Array?body:JSON.stringify(body)}),env);}
test('booking proof workflow, durable reads, access and upload limits',async()=>{
 ...
 assert.equal((await call(path,'PATCH',{status:'DITERIMA'},'provider')).status,409);
 const bytes=new Uint8Array(fs.readFileSync('assets/andallo-logo.png'));
 ...
 assert.equal((await call(path,'PATCH',{status:'DITERIMA'},'provider')).status,200);
