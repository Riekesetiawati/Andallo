...
const files={};for(const [path,type] of [['index.html','text/html; charset=utf-8'],['theme.css','text/css'],['payments.js','text/javascript'],['assets/andallo-logo.svg','image/svg+xml'],['assets/favicon.svg','image/svg+xml'],['assets/andallo-logo.png','image/png'],['assets/service-hero.png','image/png']])files['/'+path]={type,data:fs.readFileSync(path).toString('base64')};
...
if(fs.existsSync('.openai/hosting.json'))fs.copyFileSync('.openai/hosting.json','dist/.openai/hosting.json');console.log('Worker built with embedded assets and R2 storage.');
